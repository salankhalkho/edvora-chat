...
$stmt = $pdo->prepare("SELECT * FROM llm_providers WHERE is_active = 1 AND provider = 'openai' LIMIT 1");
$stmt->execute();
$provider = $stmt->fetch();

if (!$provider) {
    die("Error: Active OpenAI LLM provider not found in database.");
}
...
if (empty($apiKey)) {
    die("Error: Could not decrypt API key from database.");
}
