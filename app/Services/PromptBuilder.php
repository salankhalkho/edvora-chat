...
class PromptBuilder
{
    ...
    public static function build(
        ...
        // 7. Retrieve Master Prompt Template directly from platform_config (No hardcoding)
        $stmtPrompt = $db->query("SELECT value_text FROM platform_config WHERE key_name = 'master_prompt' LIMIT 1");
        $configRow = $stmtPrompt ? $stmtPrompt->fetch(PDO::FETCH_ASSOC) : null;
        $rawTemplate = (!empty($configRow['value_text']) && trim($configRow['value_text']) !== '')
            ? trim($configRow['value_text'])
            : self::getDefaultMasterPrompt();

        // 8. Replace Dynamic Session Placeholders
        $replacements = [
...
{{KNOWLEDGE_CONTEXT}}
2. VISITOR STATE:
- Program Interest Known: {{PROGRAM_INTEREST}}
- Lead Contact Already Captured: {{LEAD_CAPTURED}}

3. LEAD ACTIVITY (OFFERS ALREADY PRESENTED IN THIS SESSION):
- Counselor Callback: {{OFFER_CALLBACK}} (Yes/No)
- Program Prospectus / Brochure: {{OFFER_BROCHURE}} (Yes/No)
- Campus Tour: {{OFFER_TOUR}} (Yes/No)
- Scholarship Calculator: {{OFFER_SCHOLARSHIP}} (Yes/No)

4. CADENCE COUNTER:
user_message_count since last offer: {{CADENCE_COUNTER}}
...
- [h] OUT_OF_SCOPE: Casual greetings or questions unrelated to the university.
  -> Action: Be warm and redirect in "bubble_1": "I'm here to help with anything regarding {{COLLEGE_NAME}}, including programs, admissions, fees, scholarships, and campus life. What would you like to explore?"
